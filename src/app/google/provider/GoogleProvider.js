...
class GoogleProvider extends Component {
  ...
  autocompleteAddress = (address, options = {}) => {
    ...
    return new Promise((resolve, reject) => {
      if (!autocompleter) {
        return reject(
          new Error(
            'Unable to autocomplete - Google Places has not been initialized. Please call `init()` from `GoogleConsumer`.',
          ),
        )
      }
      ...
      autocompleter.getPlacePredictions(data, (results, status) => {
        if (status === autocompleteOK) return resolve(results)

        reject(new Error(`Geocode was not successful. Reason: ${status}`))
      })
    })
  }
  ...
  geocodeAddress = (location, options = {}) => {
    ...
    return new Promise((resolve, reject) => {
      if (!geocoder) {
        return reject(
          new Error(
            'Unable to geocode - Google geocoder has not been initialized. Please call `initMaps()` from `GoogleConsumer`.',
          ),
        )
      }

      geocoder.geocode(location, (results, status) => {
        if (status === geocodeOK) return resolve(results)
  ...
  componentWillUnmount() {
    ...
    if (window[initCallbackName] != null) delete window[initCallbackName]
  }
  ...
  init = () => {
    ...
    if (window.google == null) {
      if (window[initCallbackName] == null) window[initCallbackName] = this.init
  ...
  async injectGoogleMapsScript() {
    ...
    if (isAlreadyInjected)
      throw new Error(
        'Attempted to inject Google Maps script when the script has already been injected.',
      )
    ...
    if (!apiKey) {
      try {
        ...
        console.warn(
          'Unable to inject Google Maps script into webpage - no given Google API key. (keycloak.data.ENV_GOOGLE_MAPS_APIKEY)',
        )
  ...
  render() {
    return <GoogleContext.Provider value={this.state}>{this.props.children}</GoogleContext.Provider>
  }
}
