...
const reduceChildAsks = (childAsks, state) => {
  if (!childAsks || !(childAsks instanceof Array) || childAsks.length === 0) {
    return []
  }
              ...
              contexts: [
                ...(isArray(dlv(state, `${current.questionCode}.contextList.contexts`))
                  ? current.replace === true
                    ? []
                    : state[current.questionCode].contextList.contexts.filter(
                        existingLink =>
                          !current.contextList.contexts.some(
                            newLink => newLink.contextCode === existingLink.contextCode,
                          ),
                      )
                  : []),
                ...(isObject(current.contextList, { withProperty: 'contexts' })
